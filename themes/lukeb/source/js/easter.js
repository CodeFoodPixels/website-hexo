...
function loadGame() {
    ...
    var titleFont = '30px press_start_2pregular';
    ...
    function drawBackground() {
        ...
        for (var i = 0; i < tiles; i++) {
            ctx.drawImage(background.image, (i * background.width) - state.scrollVal, 0, background.width, background.height);
            ctx.drawImage(background.image, (i * background.width) - state.scrollVal, 200, background.width, background.height);
        }
    }
    ...
    function drawAsteroids() {
        ...
        state.asteroidList.forEach(function(thisAsteroid) {
            if (thisAsteroid.x < 0 - asteroid.width) {
                ...
                if (thisAsteroid.frame === (asteroid.frames*asteroid.animationDivider)) {
                    thisAsteroid.frame = 0;
                }

                ctx.drawImage(
                    asteroid.image,
                    (Math.floor(thisAsteroid.frame/asteroid.animationDivider) * asteroid.width),
                    0,
                    asteroid.width,
                    asteroid.height,
                    thisAsteroid.x,
    ...
    function collisionCheck() {
        if(state.spaceshipY < 0 || state.spaceshipY > (gameHeight - spaceship.height)) {
            return gameOver();
        }
